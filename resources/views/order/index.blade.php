                            ...
                            <tbody>
                                @foreach ($orders as $order)
                                    <tr>
                                        ...
                                        <td>{{ !empty($order->price_currency_symbol) ? $order->price_currency_symbol : '$' }}{{ number_format($order->price) }}
                                        </td>
                                        ...
                                        <td>
                                            @if (
                                                ($order->payment_type == 'PesaPal' || $order->payment_type == 'PesaPal Refund') &&
                                                    $order->payment_status == 'Succeeded' &&
                                                    ($order->pesapal_refund_status != 'Pending' || is_null($order->pesapal_refund_status)))
                                                <span class="">
                                                    <span
                                                        class="ms-1">{{ ucfirst($order->pesapal_refund_status) }}</span>
                                                </span>
                                            @else
                                                <span class="d-flex align-items-center">
                                        ...
                                        <td>
                                            ...
                                            @endif
                                            @if (
                                                $order->payment_type == 'PesaPal' &&
                                                    $order->payment_status == 'Succeeded' &&
                                                    ($order->pesapal_refund_status != 'completed' || is_null($order->pesapal_refund_status)))
                                                <div class="action-btn bg-light-secondary ms-2">
                                                    <a href="javascript:void(0)" data-size="lg"
                                                        data-url="{{ route('pesapal.refund', $order->id) }}"
                                                        data-bs-toggle="tooltip" title="{{ __('Refund') }}"
                                                        data-ajax-popup="true" data-title="{{ __('Payment Refund') }}"
                                                        class="mx-3 btn btn-sm d-inline-flex align-items-center text-white">
                                                        <i class="ti ti-receipt-refund text-dark"></i>
                                                    </a>
                                                </div>
                                            @endif
                                            <div class="action-btn bg-light-secondary ms-2">
                                                ...
                                                {!! Form::open([
                                                    'method' => 'DELETE',
                                                    'route' => ['bank_transfer.destroy', $order->id],
