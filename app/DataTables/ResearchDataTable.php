...
class ResearchDataTable extends DataTable
{
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return datatables()
            ->eloquent($query)
            ->addIndexColumn()
            ->editColumn('title', function (Research $research) {
                return '<a href="javascript:void(0)" class="btn btn-sm text-line-clamp"
                                data-url="' . route('researches.edit', $research->id) . '"
                                data-size="lg" data-ajax-popup="true"
                                data-title="' . __('Edit Research') . '">
                                ' . htmlspecialchars($research->title) . '
                            </a>';
            })
            ->addColumn('action', function (Research $research) {
                return view('libraries.research.action', compact('research'));
            })
